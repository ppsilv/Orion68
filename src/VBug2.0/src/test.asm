...
WriteChar:
            ...
            MOVE.L  #$FFF,D1 ; Contador para o delay (ajuste se precisar de mais)
.DELAY00:
            SUBQ.L  #1,D1         ; Subtrai 1 de D1 (4 ciclos)
            BNE.S   .DELAY00     ; Pula se não for zero (10 ciclos se pular, 8 se não)
            RTS
...
Meus_Servicos_Init:
    ; Instalar o tratador da TRAP #0 (Vetor 32)
    ; Cálculo: Endereço = $400 + (32 - 2) * 4 = $400 + 120 = $478
    MOVE.L  #Minha_Trap0, ($00000478)

    ; Instalar o tratador da Interrupção Level 2 da UART (Vetor 26)
    ; Cálculo: Endereço = $400 + (26 - 2) * 4 = $400 + 96 = $460
    MOVE.L  #Minha_Interrupcao_Uart, ($00000460)

Loop_Principal:
    ; ... seu código principal roda aqui ...
    TRAP    #0                  ; Dispara a Trap 0 instantaneamente!
    BRA     Loop_Principal

; -----------------------------------------------------------------------------
; Suas rotinas de tratamento localizadas na RAM:
; -----------------------------------------------------------------------------
Minha_Trap0:
    ; Executa seu serviço (Ex: Print caractere na UART)
    RTE                         ; Sempre termine com RTE!

Minha_Interrupcao_Uart:
    ; Trata o dado recebido da 16C554
    RTE