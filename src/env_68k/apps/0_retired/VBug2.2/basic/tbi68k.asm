...
*
BASIC_START:	jmp	CSTART		Cold Start entry point
...
ENDMEM	DC.L	$A0000		end of available memory
*
...
ST3	MOVE.B	#'>',D0         Prompt with a '>' and
	...
	JSR	FNDNXT		find the next line (into A1)
...
DIRECT	LEA	TAB1,A1
	LEA	TAB1_1,A2
EXEC	JSR	IGNBLK		ignore leading blanks
...
RUNSML	JSR	CHKIO		see if a control-C was pressed
	LEA	TAB2,A1 	find command in TAB2
	LEA	TAB2_1,A2
	BRA	EXEC		and execute it

GOTO	JSR	EXPR		evaluate the following expression
...
LIST	JSR	TSTNUM		see if there's a line no.
...
LS1	BCS	WSTART		warm start if we passed the end
	...
	BNE	LS3
LS2	JSR	CHKIO		if so, wait for another keypress
	BEQ	LS2
LS3	JSR	FNDLNP		find the next line
	BRA	LS1
...
PR2	JSR	TSTC		if null list and <CR>
	DC.B	CR,PR0-*
	JSR	CRLF		also give CR-LF and
	BRA	RUNNXL		execute the next line
PR0	JSR	TSTC		else is it a format?
	DC.B	'#',PR1-*
	JSR	EXPR		yes, evaluate expression
	MOVE	D0,D4		and save it as print width
	BRA	PR3		look for more to print
PR1	JSR	TSTC		is character expression? (MRL)
	DC.B	'$',PR4-*
	JSR	EXPR		yep. Evaluate expression (MRL)
	BSR	GOOUT		print low byte (MRL)
	BRA	PR3		look for more. (MRL)
PR4	JSR	QTSTG		is it a string?
	BRA.S	PR8		if not, must be an expression
PR3	JSR	TSTC		if ",", go find next
	DC.B	',',PR6-*
	JSR	FIN		in the list.
	BRA	PR0
PR6	JSR	CRLF		list ends here
	BRA	FINISH
...
FINISH	JSR	FIN		Check end of command
...
*
GOSUB	JSR	PUSHA		save the current 'FOR' parameters
	JSR	EXPR		get line number
...
RETURN	JSR	ENDCHK		there should be just a <CR>
...
NEXT	JSR	TSTV		get address of variable
...
IF	JSR	EXPR		evaluate the expression
...
IF2	MOVE.L	A0,A1
	...
	JMP	WSTART		if no next line, do a warm start
...
IP4	ADDQ.L	#4,SP		clean up the stack
	JSR	TSTC		is the next thing a comma?
	DC.B	',',IP5-*
...
LET	JSR	SETVAL		do the assignment
...
POKE	BSR	EXPR		get the memory address
	...
	BRA	FINISH
PKER	JMP	QWHAT		if no comma, say "What?"
...
EXPR2	JSR	TSTC		negative sign?
	DC.B	'-',XP21-*
	CLR.L	D0		yes, fake '0-'
	BRA	XP26
XP21	JSR	TSTC		positive sign? ignore it
	DC.B	'+',XP22-*
...
TSTV	JSR	IGNBLK
	...
	JSR	SIZE		get amount of free memory
...
RND	BSR	PARN		get the upper limit
	...
	CMP.L	#LSTROM,A1
	BCS	RA1
	LEA	BASIC_START,A1	wrap around if end of program
...
QWHAT	MOVE.L	A0,-(SP)
AWHAT	LEA	WHTMSG,A6
ERROR	JSR	PRMESG		display the error message
	MOVE.L	(SP)+,A0	restore the text pointer
...
QT2	ADDQ.L	#2,A1		skip 2 bytes on return
	JMP	(A1)		return
QT3	JSR	TSTC		is it a single quote?
	DC.B	'''',QT4-*
	MOVE.B	#'''',D0        if so, do same as above
	BRA	QT1
QT4	JSR	TSTC		is it an underline?
	DC.B	'_',QT5-*
...
*
CHKIO	JSR	GOIN		get input if possible
...
* Maybe I need to change this tobe not blocant
INC	
	MOVEM.L D1-D7/A0-A6,-(A7)
	jsr		ReadConin
	...
	MOVEM.L (A7)+,D1-D7/A0-A6
	RTS
;;;BTST	#0,$10040	is character ready?
...
TXT	EQU	$90000 ;*		Beginning of program area
	;;END
