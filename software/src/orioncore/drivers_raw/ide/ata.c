	...
	for (int i = 0; i < 1000; i++) {
		...
		if (!(status & ATA_ST_BUSY)) {
			if (status & ATA_ST_DATA_READY) {
				...
				vputs("ata: data not ready\n");
				return 0;
			}
		}
	}
	vputs("ata: timeout waiting for identify command\n");
	return 0;
}
...
int ata_wait_not_busy()
{
    ...
    while (timeout--);
    vputs("ata_wait_not_busy: timeout\n");
    return 1;
}
...
int ata_wait_for_data()
{
    ...
    vputs("ata_wait_for_data: timeout\n");
    return 1;
}
...
int ata_read_identity(void)
{
    ...
    vputs("ata_read_identity: function entry\n");

	if (ata_wait_not_busy())
    {
        vputs("ata_read_identity: ata_wait_for_data (1) error\n");
        return 1;
    }
    ...
    {
        vputs("ata_read_identity: disk status error - status = \n");
        return 1;
    }

	if (ata_wait_not_busy())
    {
        vputs("ata_read_identity: ata_wait_not_busy (2) error\n");
        return 1;
    }

	vputs("ata_read_identity: reading data....");
	uint8_t drives[1024];
