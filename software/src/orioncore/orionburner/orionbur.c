...
{
    FRESULT res = f_open(&fh->fp, filename, FA_READ);
    if (res != FR_OK) {
        return 0;
    }
    fh->size = (uint32_t) f_size(&fh->fp);
    return 1;
}
...
{
    UINT br = 0;
    FRESULT res = f_read(&fh->fp, buf, (UINT) len, &br);
    if (res != FR_OK) {
        return 0;
    }
    return (uint32_t) br;
...
static int flash_write_buffer_progress(const uint8_t *data, uint32_t len_bytes)
{
    ...
    video_puts("flash_write_buffer_progress\n"); 
    video_puts("Simulando a gravacao\n"); 
...
int orionbur()
{
    ...
    printf("Digite o nome do arquivo to burn: ");
    ...
    while(ch != 0x0D){
        ...
        i++;
        if(i>127){
            printf("\nsome error... finishing...\n");
            return 1;
        }
    }
    buffer[i]='\0';
    printf("File to burn[%s]\n",buffer);
    ...
    /* FROM HERE INTERRUPTS DISABLE NO MORE PRINTF */
    ...
    video_puts("Desabilitando interrupcao\n"); 
    old_sr = cpu_disable_interrupts();
    video_puts("Interrupcao desabilitada\n"); 
