            ...
            if (!strcasecmp(argv[0], cmd->name))
            {
                ...
                else
                {
                    if (cmd->min_args == cmd->max_args)
                        vputs("requires arguments\n");
                    else
                        vputs("requires to arguments\n");
                }
                return 1;
            }
        }
...
int load_elf_executable(int argc, char *argv[], FIL *fd)
{
    ...
    {
        vputs("Cannot read ELF file header\n");
        return -1;
    }
#ifdef DEBUG_ELF    
    else
    {
        vputs("ELF file header read, x bytes\n");
    }
#endif
    ...
    {
        vputs("Not a 32-bit ELF file.\n");
        return -1;
    }

    if (header.type != ET_EXEC)
    {
        vputs("ELF file is not an executable.\n");
        return -1;
    }

    if (header.machine != EM_68K)
    {
        vputs("ELF file is not for 68000 processor.\n");
        return -1;
    }
    ...
    while (progIndex < header.phnum)
    {
        ...
        {
            vputs("Cannot read ELF program header.\n");
            return -1;
        }
        ...
        switch (progHeader.type)
        {
            ...
            case PT_SHLIB:
            case PT_DYNAMIC:
                vputs("ELF executable is dynamically linked.\n");
                return -1;
            ...
            case PT_LOAD:
                ...
                {
                    vputs("Unable to read segment from ELF file.\n");
                    return -1;
                }

                if (progHeader.memsz > progHeader.filesz)
                {
				    vputs("Clearing x bytes BSS at 0xXX\n\r");
            ...
            case PT_INTERP:
                vputs("ELF executable requires an interpreter.\n");
                return -1;
        }
        progIndex++;
    }
#ifdef DEBUG_ELF    
    vputs("Program entry point is at 0xXX\n");
    vputs("Calling with X args\n");
    for (int i = 0; i < argc; i++)
        vputs("  arg %d : %s\n", i, argv[i]);

    vputs("Running program %s\n\n", argv[0]);
#endif
    int (*entry)(int, char**) = (int (*)(int, char**))header.entry;
    int ret = (*entry)(argc, argv);
#ifdef DEBUG_ELF    
    vputs("Program returned value X\n");
#endif
    return ret;
}
...
FRESULT open_executable_file(FIL *file, char *filename)
{
    ...
    //vputs("Unable to open file %s, trying with extensions..\n", filename);
    for (int i = 0; i < NUM_FILE_EXTENSIONS; i++)
    {
        //vputs("Trying extension %s\n", orion_extensions[i]);
   ...
   //vputs("Unable to open file with known extension\n");
    return FR_NO_FILE;
}
...
int process_command_executable(int argc, char *argv[])
{
    ...
    //for(int i=0; i < argc; i++){
    //    vputs("process_command_executable: argc=[%d] argv[%s]\n",argc,argv[i]);
    //}

    fr = open_executable_file(&file, argv[0]);
    if (fr != FR_OK)
    {
        //vputs("%s: Command not found\n", argv[0]);
        //printerro(fr);
        return 0;
    }
    ...
    if (fr == FR_OK)
    {
        ...
        {
            //vputs("ELF executable\n\n");
            load_elf_executable(argc, argv, &file);
        }
        else
        {
            vputs("unknown format.\n\n");
            vputs("It is not an executable file\n");
        }
    }
    else
    {
        vputs("It Cannot be read: ");
        printerro(fr);
    }
...
void process_command(char *argv[], int argc)
{
    if (argc > 0)
    {
        if (!process_command_builtin(argc, argv) && !process_command_executable(argc, argv))
            vputs("Unknown command.  Try 'help'.\n");
    }
}
...
int execute_cmd(char *linebuffer)
{
    ...
    //if(numargs > 0){
    //    vputs("numargs=%d args[0]=%s,args[1]=%s,args[2]=%s\n",numargs,args[0],args[1],args[2]);
    //}
...
int shell(int argc, char *argv[]){
    char buffer[128];
    vputs("SHELLV0 5/september 2026\n");
    int ret;
    while(1){    
        display_prompt();
