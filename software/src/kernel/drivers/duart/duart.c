...
static inline void duart_putc(char c) {
    /* Aguarda o buffer de transmissão ficar vazio (SRA Bit 2 = TxRDY) */
    while (!(SRA & 0x04));
    THRA = c;
}

static inline char duartA_getc(void) {
...
size_t duartA_lseek(File *file, size_t offset, int whence) {
    return (size_t)-1;
}
int duartA_close(File *file) {
    return 0;
}
int duartA_ioctl(File *file, int cmd, void *arg) {
    ...
    switch(cmd){
        ...
        duart_led_op3_off( );
            break;
        case DUART_OP3_TOGGLE:
        duart_led_op3_toggle( );
            break;
    }
    
    return -1;
}
int duartA_read(File *file, void *buffer, size_t size) {
...
int duartA_write(File *file, const void *buffer, size_t size) {
    const char *data = (const char *)buffer;

    for (size_t i = 0; i < size; i++) {
        duart_putc((uint8_t)data[i]);
    }

    return (int)size;
}

int duartA_open(File *file, const char *path, int flags) {
    file->private_data = NULL;
    file->position = 0;
    file->read  = duartA_read;
    file->write = duartA_write;
    file->close = duartA_close;
    file->ioctl = duartA_ioctl;
    file->lseek = duartA_lseek;
    duart_init_canal_a();
    return 0;
}
