	...
	for (int i = 0; i < 1000; i++) {
		...
		if (!(status & ATA_ST_BUSY)) {
			if (status & ATA_ST_DATA_READY) {
				...
				kputs("ata: data not ready\n");
				return 0;
			}
		}
	}
	kputs("ata: timeout waiting for identify command\n");
	return 0;
}
...
int ata_wait_not_busy()
{
    ...
    while (timeout--);
    kputs("ata_wait_not_busy: timeout\n");
    return 1;
}
...
int ata_wait_for_data()
{
    ...
    kputs("ata_wait_for_data: timeout\n");
    return 1;
}
...
int ata_read_identity(void)
{
    ...
    kputs("ata_read_identity: function entry\n");

	if (ata_wait_not_busy())
    {
        kputs("ata_read_identity: ata_wait_for_data (1) error\n");
        return 1;
    }
    ...
    {
        kputs("ata_read_identity: disk status error - status = \n");
        return 1;
    }

	if (ata_wait_not_busy())
    {
        kputs("ata_read_identity: ata_wait_not_busy (2) error\n");
        return 1;
    }

	kputs("ata_read_identity: reading data....");
	uint8_t drives[1024];
